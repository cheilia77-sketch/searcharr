...
class Searcharr(object):
    ...
    def _menu_keyboard(self):
        ...
        media_row = []
        if settings.radarr_enabled:
            media_row.append(self._xlate("movie_button"))
        if settings.sonarr_enabled:
            media_row.append(self._xlate("series_button"))
        if self.readarr:
    ...
    def _get_user_state(self, user_id):
        q = "SELECT state FROM user_state WHERE user_id=?;"
        qa = (user_id,)
        logger.debug(f"Executing query: [{q}] with args: [{qa}]...")
        try:
            con, cur = self._get_con_cur()
            ...
            logger.error(
                f"Error executing database query to look up user state from the database [{q}]: {e}"
            )
            return None
    ...
    def _clear_user_state(self, user_id):
        con, cur = self._get_con_cur()
        q = "DELETE FROM user_state WHERE user_id=?;"
        qa = (user_id,)
        logger.debug(f"Executing query: [{q}] with args: [{qa}]")
        try:
            with DBLOCK:
    ...
    async def _send_cross_suggestion(self, message, kind):
        ...
        if other_kind == "series" and not settings.sonarr_enabled:
            return
        if other_kind == "movie" and not settings.radarr_enabled:
            return
        keyboard = ReplyKeyboardMarkup(
            [[self._xlate(f"{other_kind}_button")]],
